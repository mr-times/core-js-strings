...
function repeatString(str, times) {
  ...
  return repeatString(str, times - 1).concat(str);
}
...
function reverseWords(str) {
  ...
  let newstring = '';
  for (let i = 0; i < words.length; i += 1) {
    newstring += `${reverseString(words[i])} `;
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    } else newString += str[i].toUpperCase();
  }
  return newString;
}
...
function extractNameFromTemplate(value) {
  let str = value.split(',');
  str = str[1].slice(0, str[1].length - 1);
  return str.trim();
}
...
function encodeToRot13(str) {
  function encode(arr) {
    ...
    for (let i = 0; i < arr.length; i += 1) {
      for (let j = 0; j < arr[i].length; j += 1) {
        const code = arr[i].charCodeAt(j);
        if (code < 78) {
          encoded += String.fromCharCode(arr[i].charCodeAt(j) + 13);
        } else if (code < 91) {
          encoded += String.fromCharCode(arr[i].charCodeAt(j) - 13);
        } else if (code < 110) {
          encoded += String.fromCharCode(arr[i].charCodeAt(j) + 13);
  ...
  if (str.at(-1) === '!') {
    ...
    const arr = strF.split(' ');
    return encode(arr).concat('!');
  }
  if (str.at(-1) === '?') {
    const strF = str.slice(0, -1);
    const arr = strF.split(' ');
...
function getCardId(value) {
  ...
  if (value.length === 3) {
    const ind = [value.slice(0, 2), value.slice(2, 3)];
    findIndex(ind);
  } else {
    const ind = [value.slice(0, 1), value.slice(1, 2)];
    findIndex(ind);
  }
  return sum;
}
